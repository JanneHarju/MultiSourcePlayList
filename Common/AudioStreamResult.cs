...
namespace PlayList
{
    public class AudioStreamResult : FileStreamResult
    {
        // default buffer size as defined in BufferedStream type
        private const int BufferSize = 0x1000;
        private string MultipartBoundary = "<qwe123>";

        public AudioStreamResult(Stream fileStream, string contentType)
            : base(fileStream, contentType)
        {
        }

        public AudioStreamResult(Stream fileStream, MediaTypeHeaderValue contentType) 
            : base(fileStream, contentType)
        {
        }
