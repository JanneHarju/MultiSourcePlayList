...
export class PlayerComponent implements OnInit, OnDestroy {
  ...
  lyrics() {
    if (this.track.Address) {
      ...
      this.lyric = '';
      this.lyricImageUrl = '';
      this.musixmatchService.search(this.track.Name).subscribe((lyrics) => {
        if (lyrics) {
  ...
  localFileAddress(track: Track): string {
    ...
    if (track.Type === 3) {
      ...
      return filePathForAzureBlop + localStorage.getItem('Folder') + '/' + track.Address + localStorage.getItem('SASToken');
    }
  }
  loadedmeadata() {
    this.audio.volume = this.volume / this.audioScale;
